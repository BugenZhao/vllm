...
pub use marker::{Attributed, AttributionMode, Marker, MarkerLike, MarkerStream, attributed};
...
fn scan_markers<'i, I: MarkerStream<'i>, M: MarkerLike>(
    ...
    while let Some(at) = input.next_candidate(markers, from) {
        for marker in markers {
            ...
            match marker.parse_marker(&mut probe) {
...
/// so the next parse can avoid rescanning the whole buffered prefix. On a
/// complete input without the marker it backtracks.
///
...
fn take_until_marker_<'i, I: MarkerStream<'i>, M: MarkerLike>(
    ...
    debug_assert!(!marker.text().is_empty());
