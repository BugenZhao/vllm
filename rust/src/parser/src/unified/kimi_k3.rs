...
// Marker spellings; the parser matches them through [`Markers`], the
// structural-tag builder emits them verbatim.
#[cfg(test)]
...
impl Markers {
    fn new(tokenizer: &dyn Tokenizer) -> Result<Self> {
        let open_id = token_id(tokenizer, OPEN)?;
        let close_id = token_id(tokenizer, CLOSE)?;
        let sep_id = token_id(tokenizer, SEP)?;
        let open =
            |tag: &str| Marker::special(OPEN, open_id).then_text(tag).then_special(SEP, sep_id);
        let close =
            |tag: &str| Marker::special(CLOSE, close_id).then_text(tag).then_special(SEP, sep_id);
        // `<|end_of_msg|>` is the stop token; tokenizers that do not expose it
        // as a token fall back to its spelling.
        let end_of_msg = match tokenizer.token_to_id(END_OF_MSG) {
            Some(id) => Marker::special(END_OF_MSG, id),
        ...
        Ok(Self {
            ...
            end_of_msg,
            call_open: Marker::special(OPEN, open_id).then_text("call"),
            call_close: close("call"),
            sep: Marker::special(SEP, sep_id),
        })
    }

    fn idle(&self) -> [&Marker; 5] {
        [
            &self.think_open,
            &self.response_open,
            &self.tools_open,
    ...
    fn reasoning(&self) -> [&Marker; 2] {
        [&self.think_close, &self.end_of_msg]
    }

    fn response(&self) -> [&Marker; 4] {
        [
            &self.response_close,
            &self.tools_open,
            &self.message_close,
    ...
    fn epilogue(&self) -> [&Marker; 3] {
        [&self.tools_open, &self.message_close, &self.end_of_msg]
    }

    fn tools(&self) -> [&Marker; 4] {
        [
            &self.call_open,
            &self.tools_close,
            &self.message_close,
...
pub struct KimiK3UnifiedParser {
    ...
    /// Whether any text / any attribution arrived in this request; used to
    /// assert the [`AttributionMode::Tokens`] contract in debug builds.
    saw_text: bool,
    saw_attribution: bool,
...
impl KimiK3UnifiedParser {
    ...
    pub fn new(_tools: &[Tool], tokenizer: DynTokenizer) -> Result<Self> {
        ...
        let markers = Markers::new(tokenizer.as_ref())?;
...
impl UnifiedParser for KimiK3UnifiedParser {
    ...
    fn parse_into(&mut self, delta: DecodedText, output: &mut UnifiedParserOutput) -> Result<()> {
        self.saw_text |= !delta.text.is_empty();
        self.saw_attribution |= !delta.attributions.is_empty();
        ...
        loop {
            ...
            let parsed = parse_buffered_event(attributed(buffer, *attribution), |input| {
                parse_next_kimi_k3_event(input, mode, markers)
            })?;
    ...
    fn finish(&mut self) -> Result<UnifiedParserOutput> {
        debug_assert!(
            self.attribution == AttributionMode::TextOnly || !self.saw_text || self.saw_attribution,
            "Kimi K3 parser in AttributionMode::Tokens received unattributed text; \
             construct it with AttributionMode::TextOnly for text-only callers"
        );
...
fn parse_next_kimi_k3_event(
    ...
    match mode {
        KimiK3Mode::Idle => parse_idle_event(input, markers),
        KimiK3Mode::Reasoning => parse_reasoning_event(input, markers),
        KimiK3Mode::Response => parse_response_event(input, markers),
        KimiK3Mode::Epilogue => parse_epilogue_event(input, markers),
        KimiK3Mode::Tools => parse_tools_event(input, markers),
        KimiK3Mode::Call { scan, .. } => call_body_event(input, markers, scan),
        KimiK3Mode::Done => parse_done_event(input),
    }
}

/// Parse an event while waiting for the next channel open.
fn parse_idle_event(input: &mut KimiK3Input<'_>, markers: &Markers) -> ModalResult<KimiK3Event> {
    alt((
        ...
        |input: &mut KimiK3Input<'_>| message_end_event(input, markers),
        |input: &mut KimiK3Input<'_>| safe_idle_text_event(input, markers),
    ))
    .parse_next(input)
}
...
fn parse_reasoning_event(
    ...
    alt((
        ...
        |input: &mut KimiK3Input<'_>| safe_reasoning_event(input, markers),
    ))
    .parse_next(input)
}
...
fn parse_response_event(
    ...
    alt((
        ...
        markers.tools_open.value(KimiK3Event::ToolsOpen),
        |input: &mut KimiK3Input<'_>| message_end_event(input, markers),
        |input: &mut KimiK3Input<'_>| safe_response_text_event(input, markers),
    ))
    .parse_next(input)
}

/// Parse an event after the response channel closed.
fn parse_epilogue_event(
    input: &mut KimiK3Input<'_>,
    markers: &Markers,
) -> ModalResult<KimiK3Event> {
    alt((
        markers.tools_open.value(KimiK3Event::ToolsOpen),
        |input: &mut KimiK3Input<'_>| message_end_event(input, markers),
        |input: &mut KimiK3Input<'_>| skip_epilogue_noise_event(input, markers),
    ))
    .parse_next(input)
}

/// Parse an event inside the `tools` channel, between `call` blocks.
fn parse_tools_event(input: &mut KimiK3Input<'_>, markers: &Markers) -> ModalResult<KimiK3Event> {
    alt((
        |input: &mut KimiK3Input<'_>| call_open_event(input, markers),
        markers.tools_close.value(KimiK3Event::ToolsClose),
        // Defensive: an unterminated tools channel still ends with the message.
        |input: &mut KimiK3Input<'_>| message_end_event(input, markers),
        |input: &mut KimiK3Input<'_>| skip_tools_noise_event(input, markers),
    ))
    .parse_next(input)
}

/// Parse a message close or end-of-message marker.
fn message_end_event(input: &mut KimiK3Input<'_>, markers: &Markers) -> ModalResult<KimiK3Event> {
...
fn safe_idle_text_event(
    input: &mut KimiK3Input<'_>,
    markers: &Markers,
) -> ModalResult<KimiK3Event> {
    safe_text_len_mul(input, &markers.idle()).map(|_| KimiK3Event::Text)
}

/// Parse safe reasoning before the think close marker.
fn safe_reasoning_event(
    input: &mut KimiK3Input<'_>,
    markers: &Markers,
) -> ModalResult<KimiK3Event> {
    safe_text_len_mul(input, &markers.reasoning()).map(|_| KimiK3Event::Reasoning)
}

/// Parse safe response text before the next channel marker.
fn safe_response_text_event(
    input: &mut KimiK3Input<'_>,
    markers: &Markers,
) -> ModalResult<KimiK3Event> {
    safe_text_len_mul(input, &markers.response()).map(|_| KimiK3Event::Text)
}

/// Skip non-content noise after the response channel closed.
fn skip_epilogue_noise_event(
    input: &mut KimiK3Input<'_>,
    markers: &Markers,
) -> ModalResult<KimiK3Event> {
    safe_text_len_mul(input, &markers.epilogue()).map(|_| KimiK3Event::Skip)
}

/// Skip non-content noise between `call` blocks.
fn skip_tools_noise_event(
    input: &mut KimiK3Input<'_>,
    markers: &Markers,
) -> ModalResult<KimiK3Event> {
    safe_text_len_mul(input, &markers.tools()).map(|_| KimiK3Event::Skip)
}
...
fn call_open_event(input: &mut KimiK3Input<'_>, markers: &Markers) -> ModalResult<KimiK3Event> {
...
mod tests {
    ...
    /// The spelling-based parser: the historical behaviour, and the fallback
    /// for callers without token attribution.
    fn test_parser() -> KimiK3UnifiedParser {
        KimiK3UnifiedParser::new(&[], Arc::new(tokenizer()))
            .unwrap()
            .with_attribution_mode(AttributionMode::TextOnly)
    }
    ...
    fn attributed_parser() -> KimiK3UnifiedParser {
    ...
    fn kimi_k3_markers_spell_the_documented_strings() {
        let markers = super::Markers::new(&tokenizer()).unwrap();
    ...
    fn kimi_k3_ordinary_close_marker_text_stays_in_reasoning() {
        ...
        let mut parser = attributed_parser();
        start_in_reasoning(&mut parser);
        let output = parse_attributed(&mut parser, decoded);
    ...
    fn kimi_k3_ordinary_open_marker_text_stays_in_reasoning() {
        ...
        let mut parser = attributed_parser();
        start_in_reasoning(&mut parser);
        let output = parse_attributed(&mut parser, decode_ids(&ids));
    ...
    #[test]
    fn kimi_k3_structural_markers_drive_transitions_from_idle() {
        let text = thinking_output(
            "Need the weather tool.",
            "I'll check.",
            &call(
        ...
        let output = parse_attributed(&mut attributed_parser(), decode_ids(&structural(&text)));

        assert_eq!(output.reasoning_text(), "Need the weather tool.");
        assert_eq!(output.normal_text(), "I'll check.");
        assert_eq!(first_call(&output).name.as_deref(), Some("get_weather"));
        assert_eq!(first_call(&output).arguments, r#"{"city":"Hangzhou"}"#);
    }
    ...
    fn kimi_k3_ordinary_sep_text_inside_call_attributes_stays_attribute_text() {
        ...
        let output = parse_attributed(&mut attributed_parser(), decode_ids(&ids));
    ...
    #[test]
    #[should_panic(expected = "AttributionMode::Tokens received unattributed text")]
    fn kimi_k3_tokens_mode_rejects_unattributed_text() {
        let mut parser = attributed_parser();
        parser.parse_chunk("hello").unwrap();
        let _ = parser.finish();
    }
}
