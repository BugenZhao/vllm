...
impl<T: Tokenizer + ?Sized> DecodeStream<'_, T> {
    ...
    /// consistent with the joint one. Decoded text is never changed by this;
    /// only anchor placement is.
    fn pending_group_split(&mut self, token_id: u32, new_chunk: &str) -> Result<Option<usize>> {
...
impl<T: Tokenizer + ?Sized> IncrementalDecoder for DecodeStream<'_, T> {
    fn push_token(&mut self, token_id: u32) -> Result<usize> {
        ...
        let split = if zero_width {
            ...
            self.pending_group_split(token_id, new_chunk)?
        };
        match split {
            Some(split) => {
